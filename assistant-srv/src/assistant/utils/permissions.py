...
def require_owner_or_admin(func: Callable) -> Callable:
    ...
    async def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        if target_user_id is None:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="User ID required"
            )
...
def require_session_owner_or_admin(func: Callable) -> Callable:
    """装饰器：要求用户是会话所有者或管理员"""
    ...
    async def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        if session is None:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Session required"
            )
...
def require_session_token_owner_or_admin(func: Callable) -> Callable:
