...
2. **强制使用语义化类名**
   - 按钮组件：使用 btn-base、btn-primary、btn-secondary 等
   - 布局容器：使用 centered-container、grid-responsive 等
   - 状态类：使用 loading-state、error-state、success-state 等
   - 禁止直接在JSX中使用长串原子类
...
遵循这些规则，确保生成的CSS代码符合项目架构标准，具有良好的可维护性和扩展性。
```
...
*本指南作为项目CSS开发的核心标准，所有样式相关的开发和重构都应参考此文档。*
